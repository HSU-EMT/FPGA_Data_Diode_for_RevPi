				       ...
				       struct revpi_gate_connection *conn)
{
	...
	if (data_diode_mode == true){	//////  to save the data of data diode to corresponding are of process image.

		status = dd_write_to_process_image(rcv_al->i8uData,rcv_al->i16uDataLen,&image_ready);
		if(status != DD_SUCCESS) {
			pr_err("data_diode write failed\n");
			goto drop;
		}
		...
		if (skb)
			memset(al->i8uData, DATA_DIODE_ACK, conn->out_len);
	} else {
...
static int revpi_gate_process_id_resp(struct sk_buff *rcv,    //////Master receives Gateway information device and answer to Gateway
				      struct net_device *dev,
				      struct revpi_gate_connection *conn)
{
	MODGATECOM_TransportLayer *rcv_tl;
	MODGATECOM_IDResp *al, *rcv_al; ///// do not mix up with MODGATECOM_CyclicPD
	struct sk_buff *skb;
	u8 i;
	...
	i = revpi_core_find_gate(dev, rcv_al->i16uModulType); //index of gateway left or right to the controller
	if (i == REV_PI_DEV_UNDEF) {
		pr_warn("%s: no process image synchronization\n", dev->name);
		pr_warn("This warning can be ignored in case of data diode\n");
			//if(rcv_al->i16uModulType == EMT_DATA_DIODE) { //data diode 					
				//conn->revpi_dev->sId = *rcv_al; //problem
			//}
 
	} else {
		conn->revpi_dev = RevPiDevice_getDev(i);					////// add new device to index list?
	...
	al = (MODGATECOM_IDResp *)skb_put(skb, sizeof(*al)); ///////Master answer to Gateway about its information device
	...
	revpi_core_gate_connected(conn->revpi_dev, true);				///// should delete for data diode or not?. The following lines still work fine? Have to check again!
...
static int revpi_gate_process_id_req(struct sk_buff *rcv, //////Master send id request to Gateway and wait to coming packet of Gateway
				     struct net_device *dev,
				     struct revpi_gate_connection *conn)
{
	MODGATECOM_TransportLayer *rcv_tl;
